...
const FilePreview = forwardRef<FilePreviewRef, FilePreviewProps>(
  (
    ...
    // Use browser-compatible timeout handle type
      ...
      const loadContent = async () => {
        ...
        try {
          ...
          if (mime === 'text/markdown') {
            ...
            setContent(text);
            // revoke any previous objectUrl if present
            if (objectUrl) {
    ...
    const handleSelectionEvent = useCallback(
      (event: Event | MouseEvent | TouchEvent) => {
        ...
        // Prevent context menu on touch devices (only if preventDefault exists)
        try {
          if ((event as TouchEvent).type === 'touchend' && typeof (event as TouchEvent).preventDefault === 'function') {
    ...
    useEffect(() => {
      ...
      // Use DOM native listeners
      document.addEventListener('selectionchange', handleSelectionChange);
      container.addEventListener('mouseup', handleSelectionEvent as EventListener);
      container.addEventListener('touchend', handleSelectionEvent as EventListener);

      return () => {
        document.removeEventListener('selectionchange', handleSelectionChange);
        container.removeEventListener('mouseup', handleSelectionEvent as EventListener);
        container.removeEventListener('touchend', handleSelectionEvent as EventListener);
      ...
      try {
        ...
        return (
          <div ref={containerRef} className="w-full h-full overflow-y-auto no-scrollbar selectable">
            ...
            {itemType === 'INTERACTIVE_EXAM' && (
              <ExamContainer lectures={lectures} onStateChange={onExamStateChange} fileItemId={itemId} />
            )}
    ...
    if (mime?.startsWith('image/')) {
      return (
        <div ref={containerRef} className={cn(baseClass, 'w-full h-full overflow-auto flex items-center justify-center p-4 md:p-8')}>
    ...
    if (mime?.startsWith('audio/')) {
      return (
        <div ref={containerRef} className={cn(baseClass, 'w-full h-full flex items-center justify-center p-4')}>
    ...
    if (mime?.startsWith('video/')) {
      return (
        <div ref={containerRef} className={cn(baseClass, 'w-full h-full flex items-center justify-center bg-black')}>
          <video controls src={contentUrl} className="max-w-full max-h-full" />
        </div>
      );
    }
    ...
    if (mime === 'text/html') {
      return (
        <iframe
          ref={iframeRef}
          src={contentUrl}
          className={cn('w-full h-full border-2 border-slate-700 rounded-lg bg-white text-black shadow-lg', baseClass)}
          title={itemName}
    ...
    if (mime?.startsWith('text/')) {
      return (
        <iframe
          ref={iframeRef}
          src={contentUrl}
          className={cn(baseClass, 'w-full h-full border-2 border-slate-700 rounded-lg bg-slate-800 text-white shadow-lg')}
          title={itemName}
        />
      );
    }
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full text-center text-slate-300 bg-slate-800/50 rounded-lg p-8">
        ...
        <p className="text-base mb-4 text-slate-400">
          ...
          <code className="bg-slate-900 px-2 py-1 rounded-md text-slate-300">{mime}</code>
        </p>
        <a href={url} download={itemName} className="mt-4 px-5 py-2.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors font-medium">
          Download File
        </a>
      </div>
    );
